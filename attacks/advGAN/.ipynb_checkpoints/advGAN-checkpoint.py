...
class AdvGAN_Attack:
    ...
    def train_batch(self, x, angle_speed):
        # optimize D
        for i in range(1):
        ...
        for i in range(1):
            ...
            loss_adv = F.mse_loss(predicted_angle_speed, angle_speed)
    ...
    def train(self, train_dataset, epochs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for _, sample in enumerate(train_dataloader, start=0):
                ...
                batch_x, angle, speed, _, curve = sample
                if not curve:
                    continue
                
                batch_x = batch_x.type(torch.FloatTensor)
                batch_x = batch_x.to(self.device)
                predicted_angle_speed = self.stage1(batch_x)
                predicted_angle_speed = predicted_angle_speed.detach().cpu().data.numpy()
                target_predicted_angle_speed = np.array([[predicted_angle_speed[0][0],predicted_angle_speed[0][1]+self.target]])
                target_predicted_angle_speed = torch.from_numpy(target_predicted_angle_speed)
                target_predicted_angle_speed = target_predicted_angle_speed.type(torch.FloatTensor)
                target_predicted_angle_speed = target_predicted_angle_speed.to(self.device)

                loss_D_batch, loss_G_fake_batch, loss_perturb_batch, loss_adv_batch = self.train_batch(
                    batch_x, target_predicted_angle_speed
                )
